...
y **verificado con una actualización real 0.1.0 → 0.2.0** sobre la app instalada:
...
### Estado actual del canal en producción

Ejecutada sin override, la app instalada responde:
...
Es correcto y esperado: **el release `eventra-mobile` todavía no existe en GitHub**.
El mecanismo está listo; falta publicar. Ver §6.
...
Lo único que separa a la app de actualizarse sola en manos de un usuario real es
**publicar el primer release** (paso manual de Brian, requiere login en GitHub).

Estado de la máquina al terminar: **Eventra Mobile 0.1.0 instalada, build de
producción limpia**, sin ficheros de prueba, servidor local detenido. Versiones
en el repo de nuevo en `0.1.0` (el bump a 0.2.0 fue solo para la prueba).
...
Requiere una sola acción manual previa, **una vez**:

1. En GitHub → Settings → Secrets and variables → Actions, crear el secreto
   `TAURI_SIGNING_PRIVATE_KEY` con el contenido íntegro de
   `C:\Users\carlo\.eventra-keys\eventra-mobile.key`.
   (Sin secreto de contraseña: la clave no tiene.)

Después, cada versión:

2. Subir `version` en `src-tauri/tauri.conf.json` **y** en `src-tauri/Cargo.toml`.
3. `git tag eventra-mobile-v0.2.0 && git push origin eventra-mobile-v0.2.0`.
...
1. **Publicar el primer release** y repetir la prueba contra el endpoint real de
   GitHub (HTTPS). Es el último eslabón sin verificar en producción.
